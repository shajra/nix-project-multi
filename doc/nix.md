...
[This project's continuous integration (using GitHub Actions)](https://github.com/shajra/nix-project-multi/actions) caches built packages at [Cachix](https://cachix.org), a service for caching pre-built Nix packages. If you don't want to wait for a full local build when first using this project, setting up Nix to pull from Cachix is recommended.
...
```
    ...
    * nix-project-multi (nix-project-multi)
...
We can see that a package named "nix-project-multi" can be accessed with the `nix-project-multi` attribute path in the Nix expression in the project root's `default.nix`. Not shown in the search results above, this package happens to provide the executable `nix-project-multi`.

We can build this package with `nix build` from the project root:

```sh
nix build --file . nix-project-multi
```
...
```
    ...
    /nix/store/pz9llxfdyifq3237bs8nwmsl7nrwhqjg-nix-project-multi
...
```sh
nix path-info --file . nix-project-multi
```
...
For example, to get the help message for the `nix-project-multi` executable provided by the `nix-project-multi` package selected by the `nix-project-multi` attribute path from `.`, we can call the following:

```sh
nix run \
    --file . \
    nix-project-multi \
    --command nix-project-multi --help
```
...
To install the `nix-project-multi` executable, which is accessed by the `nix-project-multi` in our top-level `default.nix` file, we'd run the following:
...
And if we want to uninstall a program from our active profile, we do so by its name, in this case "nix-project-multi":

```sh
nix-env --uninstall nix-project-multi 2>&1
```

    uninstalling 'nix-project-multi'

Note that we've installed our package using its attribute path (`nix-project-multi`) within the referenced Nix expression. But we uninstall it using the package name ("nix-project-multi"), which may or may not be the same as the attribute path. When a package is installed, Nix keeps no reference to the expression that evaluated to the derivation of the installed package. The attribute path is only relevant to this expression. In fact, two different expressions could evaluate to the same derivation, but use different attribute paths. This is why we uninstall packages by their package name.
